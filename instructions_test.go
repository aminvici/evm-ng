...
func TestCreate2Addreses(t *testing.T) {
	...
	for i, tt := range []testcase{
		...
		origin := util.BytesToAddress(common.FromHex(tt.origin))
		salt := util.BytesToHash(common.FromHex(tt.salt))
		...
		*/
		expected := util.BytesToAddress(common.FromHex(tt.expected))
		if !bytes.Equal(expected[:], address[:]) {
			t.Errorf("test %d: expected %x, got %s", i, expected, address)
		}
